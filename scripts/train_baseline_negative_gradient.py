#!/usr/bin/env python
"""Baseline: Negative gradient style (push target away)."""
...
def main():
    ...
    ap.add_argument("--lambda_neg", type=float, default=1.0, help="Negative gradient weight")
    ...
    print(f"[INFO] Negative gradient baseline (push target away)")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, batch in enumerate(loader, start=1):
            ...
            n_target = int(is_target.sum().item())
            ...
            with amp.autocast(enabled=(device == "cuda")):
                ...
                # Standard triplet for non-target
                loss_metric, dist_ap, dist_an = triplet_loss_fn(z, label)

                # Negative gradient: push target away from non-target
                loss_neg = torch.tensor(0.0, device=device)
                if n_target > 0:
                    z_target = z[is_target == 1]
                    z_non_target = z[is_target == 0]
                    if len(z_non_target) > 0:
                        # Maximize distance between target and non-target
                        z_t_norm = F.normalize(z_target, p=2, dim=1)
                        z_nt_norm = F.normalize(z_non_target, p=2, dim=1)
                        # Average similarity (to minimize)
                        sim = torch.matmul(z_t_norm, z_nt_norm.t())
                        loss_neg = sim.mean()

                loss = args.lambda_id * loss_id + args.lambda_tri * loss_metric - args.lambda_neg * loss_neg
